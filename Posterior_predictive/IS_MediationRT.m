%IS_MediationRT.m   Evaluated in-sample prediction
...
%  11/25/18     Michael Nunez      Change from IS_Mediation.m
%  11/26/18     Michael Nunez        Fix load location & RT sampler
%  11/27/18     Michael Nunez        Clean up output
...
load('../Results/Joint_MediationRT.mat');
...
RT_mu = zeros(chainLength,N,14);
IQ_theta = zeros(chainLength,6);
ERP_theta = zeros(chainLength,18);
RTy_theta = zeros(chainLength,N,14);


if exist('../Results/Eval_MedRT_IS.mat') ~= 2
    rng(13);
    for i = 1:N
        for k = 1:6
        ...
        for k = 1:14
            RT_mu(:,i,k) = chains.(sprintf('RTmu_%d_%d',k,i))(:);
            RTy_theta(:,i,k) = chains.(sprintf('RTytheta_%d_%d',k,i))(:);
        end
    ...
    for c = 1:nSim
        ...
        for i = 1:length(task)
            ...
            RTdata(c,i) = normrnd(RT_mu(selectedChains(c),person(i),task(i)), ...
                sqrt(RTy_theta(selectedChains(c),person(i),task(i)))); %Use standard deviation
        end
    end

    RTre = RTdata';
    save('../Results/Eval_MedRT_IS.mat','IQ','RTre','ERPdata');
else
    fprintf('Loading found simulated data...\n');
    load('../Results/Eval_MedRT_IS.mat');
end
...
fprintf('Effect (median posterior and 95%% credible interval) of latent reaction time on latent cognitive ability: %.2f, CI: [%.2f, %.2f]\n',beta2stats(2),beta2stats(1),beta2stats(3));

beta1 = prctile(chains.beta_1(:),[2.5 50 97.5]);
fprintf('Effect (median posterior and 95%% credible interval) of latent neural processing on latent reaction time: %.2f, CI: [%.2f, %.2f]\n',beta1(2),beta1(1),beta1(3));